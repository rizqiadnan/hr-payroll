...
$pdf->Cell(0, 0.7, 'Laporan Data Karyawan', 0, 0, 'C');
$pdf->ln(1);
$pdf->SetFont('Arial', 'B', 10);
...
$pdf->Cell(5, 0.8, 'No Telepon', 1, 0, 'C');
$pdf->Cell(5, 0.8, 'Status', 1, 1, 'C');

$no = 1;
$query = mysql_query("SELECT karyawan.id_karyawan, karyawan.foto, karyawan.nama_karyawan, jabatan.nama_jabatan, karyawan.alamat, karyawan.no_telepon, karyawan.status from penggajian.jabatan inner join penggajian.karyawan on (jabatan.id_jabatan = karyawan.id_jabatan) ORDER BY id_karyawan asc");
while ($lihat = mysql_fetch_array($query)) {
	$pdf->Cell(1, 0.8, $no, 1, 0, 'C');
	$pdf->Cell(5, 0.8, $lihat['nama_karyawan'], 1, 0, 'C');
	$pdf->Cell(5, 0.8, $lihat['nama_jabatan'], 1, 0, 'C');
	$pdf->Cell(5, 0.8, $lihat['no_telepon'], 1, 0, 'C');
	$pdf->Cell(5, 0.8, $lihat['status'], 1, 1, 'C');
